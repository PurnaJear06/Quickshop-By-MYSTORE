...
class UserViewModel: ObservableObject {
    ...
    func addAddress(title: String, fullAddress: String, landmark: String?, latitude: Double = 0.0, longitude: Double = 0.0, isDefault: Bool = false, completion: @escaping (Bool) -> Void) {
        ...
        let newAddress = Address(
            ...
            fullAddress: fullAddress,
            landmark: landmark,
            isDefault: isDefault || user.addresses.isEmpty,
            latitude: latitude,
            longitude: longitude
        )
        
        // If new address is default, update others
        var addresses = user.addresses.map { addr in
            Address(
                id: addr.id,
                title: addr.title,
        ...
        db.collection("users").document(firebaseUser.uid).updateData([
            ...
            DispatchQueue.main.async {
                ...
                if error == nil {
                    ...
                    print("✅ Address saved to Firebase: \(title)")
                    completion(true)
                } else {
