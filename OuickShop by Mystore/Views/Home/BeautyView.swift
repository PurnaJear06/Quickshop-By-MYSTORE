...
struct BeautyView: View {
            ...
            ZStack(alignment: .top) {
                ...
                ScrollView {
                    ...
                    GeometryReader { proxy in
                        Color.clear.preference(key: ScrollOffsetPreferenceKey.self, value: proxy.frame(in: .named("beautyScroll")).minY)
                    }
                    .frame(height: 0)
                    ...
                    VStack(spacing: 0) {
                        ...
                        // Content section using BeautyContentView
                        BeautyContentView()
                            .environmentObject(cartViewModel)
                            .environmentObject(homeViewModel)
                    }
                }
                .coordinateSpace(name: "beautyScroll")
                ...
                HStack(spacing: 0) {
                    ForEach(categories.indices, id: \.self) { index in
                        CategoryTabView(
                            ...
                            icon: icons[index],
                            color: colors[index],
                            isSelected: categories[index] == "Beauty",
                            isBeautyMode: true,
    ...
    private var categoryCardsSection: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 15) {
                // Face Serums
                categoryBannerCard(image: "beauty_banner_2", title: "Face Serums")
                
                // Fask Masks
                categoryBannerCard(image: "beauty_banner_4", title: "Fask Masks")
                
                // Nail Polish
                categoryBannerCard(image: "beauty_banner_3", title: "Nail Polish")
                
                // Face Wash
                categoryBannerCard(image: "beauty_banner_1", title: "Face Wash")
            }
            .padding(.horizontal, 16)
        }
