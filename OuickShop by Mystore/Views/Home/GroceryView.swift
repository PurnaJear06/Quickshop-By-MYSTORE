...
struct GroceryView: View {
            ...
            ZStack(alignment: .top) {
                ...
                ScrollView {
                    ...
                    GeometryReader { proxy in
                        Color.clear.preference(key: ScrollOffsetPreferenceKey.self, value: proxy.frame(in: .named("groceryScroll")).minY)
                    }
                    .frame(height: 0)
                    ...
                    VStack(spacing: 0) {
                        ...
                        categoryCardsSection
                            ...
                            .padding(.bottom, 15)
                                            ...
                                            // Content section using GroceryContentView
                    GroceryContentView()
                            .environmentObject(cartViewModel)
                            .environmentObject(homeViewModel)
                    }
                }
                .coordinateSpace(name: "groceryScroll")
                ...
                HStack(spacing: 0) {
                    ForEach(categories.indices, id: \.self) { index in
                        CategoryTabView(
                            ...
                            icon: icons[index],
                            color: colors[index],
                                                    isSelected: categories[index] == "Grocery",
                        isGroceryMode: true,
    ...
    private var categoryCardsSection: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 15) {
                                  categoryBannerCard(
                      image: "grocery_banner_1",
                      title: "Spices"
                  )
                  
                  categoryBannerCard(
                      image: "grocery_banner_2",
                      title: "Flours & Cooking Oils"
                  )
                  
                  categoryBannerCard(
                      image: "grocery_banner_4",
                      title: "Pickles & Sauces"
                  )
                  
                  categoryBannerCard(
                      image: "grocery_banner_3",
                      title: "Rice & Pulses"
                  )
            }
            .padding(.horizontal, 16)
            ...
            Button(action: action) {
                VStack(spacing: 4) {
                    ZStack {
                        if isSelected {
                            Circle()
                                .fill(isGroceryMode ? Color.white.opacity(0.3) : color.opacity(0.2))
...
// Helper function to get top safe area inset
